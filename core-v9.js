...
(function(){
  ...
  /* Testimonial arrows */
  /* D360: the Zubilant Google reviews were removed here. A rating and a
     testimonial belong to the brand that earned them. Real school
     testimonials go in when they arrive (board 4.1 #9). */
  var TESTS=[];
...
(function(){
  ...
  if(!/^p-.+\.html$/.test(page)) return;              /* programme pages only */
  ...
  function journeyTitle(){
    /* D360: the CTA carries ?programme=. The fallback is the eyebrow, which is
       the programme's NAME. Falling back to the h1 the way Zubilant did would
       put a headline in the pill ("The learning happens after dark"), which
       then rides into the email subject line. Caught by the QA harness. */
    var a=document.querySelector(SEL); if(!a) return '';
    var m=/[?&](?:programme|journey)=([^&#]*)/.exec(a.getAttribute('href')||'');
    if(m){ try{ return decodeURIComponent(m[1].replace(/\+/g,' ')); }catch(e){ return ''; } }
    var eb=document.querySelector('.hero .eyebrow');
    if(eb) return eb.textContent.trim();
  ...
  function build(){
    ...
    bd.innerHTML='<div class="em">'+
      ...
      '<div class="emhead">'+
        '<div class="label ink500">Request a callback</div>'+
        '<h3 id="jemt" class="mt8">Tell us about the group.</h3>'+
        (t?'<div class="empill">Programme: <b>'+esc(t)+'</b></div>':'')+
        '<p class="ink700 mt8" style="font-size:15px">Four fields, two minutes. We call you back, we do not put you in a mailing list.</p>'+
      '</div>'+
      '<form class="fgrid" method="post" action="/api/enquiry">'+
        ...
        '<div class="field"><label for="jf-school">School <em class="opt">optional</em></label><input type="text" id="jf-school" name="school" autocomplete="organization" maxlength="120" placeholder="ex: Children&rsquo;s Academy, Malad"></div>'+
        ...
        '<div class="field"><label for="jf-pax">Roughly how many students <em class="opt">optional</em></label><input type="text" id="jf-pax" name="students" maxlength="60" placeholder="ex: 40 students, 3 teachers"></div>'+
        '<div class="field full"><label for="jf-when">Roughly when <em class="opt">optional</em></label><input type="text" id="jf-when" name="when" maxlength="120" placeholder="ex: second week of December, 4 nights"></div>'+
        '<div class="field full"><label for="jf-notes">Anything we should know <em class="opt">optional</em></label>'+
          '<textarea id="jf-notes" name="notes" maxlength="2000" placeholder="Year group, the subject or syllabus link, a good time to call, anything the school needs us to plan around."></textarea></div>'+
        '<div class="emfoot full"><button type="submit" class="btn btn-primary">Request a callback <span class="arw" aria-hidden="true">&rarr;</span></button>'+
        '<small>Nothing is booked until the school says so.</small></div>'+
      '</form>'+
...
/* ── ENQUIRY FORMS r3 (24 Aug) ─ validation + direct email delivery.
   ...
   makes FormSubmit email that inbox an activation link; until someone clicks
   it, sends fail and the one-tap rescue below shows instead, so no enquiry is
  ...
  function init(){
    document.querySelectorAll('form[action="/api/enquiry"]').forEach(function(f){
      ...
      f.addEventListener('submit',function(ev){
        ...
        var txt='New SCHOOL enquiry via destinationz360.in\n'+pairs.join('\n');
        ...
        function done(sent){
          ...
          if(sent){
            p.innerHTML='<h4>Sent. Thank you.</h4>'+
              '<p class="ink700" style="font-size:15.5px">A coordinator calls you back. Nothing is booked until the school says so.</p>';
          }else{
            p.innerHTML='<h4>That didn&rsquo;t go through. One tap sends it.</h4>'+
              '<p class="ink700" style="font-size:15px">Your details are written out below. Send them on WhatsApp or by email and a person replies there.</p>'+
              '<div class="sum"></div>'+
            ...
            p.querySelector('.btn-secondary').href='mailto:'+TO+'?subject='+encodeURIComponent('[D360] Website enquiry')+'&body='+encodeURIComponent(txt);
            p.querySelector('.editl').addEventListener('click',function(){f.classList.remove('zsend');p.remove();});
          }
          f.classList.add('zsend');
          if(window.dataLayer)window.dataLayer.push({event:sent?'enquiry_sent_direct':'enquiry_rescue_shown',page:location.pathname});
        ...
        try{
          ...
          var jn=dj;
          if(!jn){try{var qs=new URLSearchParams(location.search);jn=qs.get('programme')||qs.get('journey')||'';}catch(e){}}
          if(dj) fd.append('journey',dj);
          fd.append('brand','Destinationz 360');
          fd.append('_subject','[D360] New school enquiry'+(jn?': '+jn:''));
          ...
          fetch(EP,{method:'POST',body:fd,headers:{'Accept':'application/json'},signal:ctrl?ctrl.signal:undefined})
            ...
            .then(function(j){if(j&&(j.success===true||j.success==='true'))done(true);else done(false);})
